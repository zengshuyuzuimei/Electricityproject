...
public class PageModel implements PageContract.PageModel {
    ...
    @Override
    public void request(int userId, final String sessionId, int status, final PageCallBack pageCallBack) {
        Retrofit retrofit = new Retrofit.Builder()
                .baseUrl(Constant.BASE_URL)
                .addCallAdapterFactory(RxJava2CallAdapterFactory.create())
                .build();
        ApiService apiService = retrofit.create(ApiService.class);
        apiService.sendStatus(userId,sessionId,status,1,5)
