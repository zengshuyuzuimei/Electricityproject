 ...
 *@Description:
 * */
public class ShoppingChildAdapter extends BaseQuickAdapter<StatusBean.OrderListBean.DetailListBean, BaseViewHolder> {
    public ShoppingChildAdapter(int layoutResId, @Nullable List<StatusBean.OrderListBean.DetailListBean> data) {
        super(layoutResId, data);
    }

    @Override
    protected void convert(BaseViewHolder helper, final StatusBean.OrderListBean.DetailListBean item) {
        final ImageView img = helper.getView(R.id.shopping_img);
        helper.setText(R.id.shopping_name,item.getCommodityName());
        helper.setText(R.id.shopping_price,item.getCommodityPrice()+"");
        ...
        final SubView subView = helper.getView(R.id.shopping_subview);
